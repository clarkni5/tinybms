...
int readRegistersWithRetry(uint16_t idx, uint16_t count, uint16_t *dest, uint8_t retrcnt) {
	...
	do {
		...
		if ((result = modbus->readHoldingRegisters(idx, count)) == modbus->ku8MBSuccess) {
			uint16_t j = 0;
			for (j = 0; j < count; j++) {
...
int load_battery_voltage(Battery_config *config, Battery_voltage *voltage) {
	...
	if (readRegistersWithRetry(MAX_CELL_COUNT - config->cell_count, config->cell_count,
			voltage->cell_voltage, MODBUS_RETRY_COUNT) <= 0) {

		result = -1;
	}

	if (readRegistersWithRetry(PACK_VOLTAGE_REGISTER_0, 2,
			voltage->pack_voltage.ivoltage, MODBUS_RETRY_COUNT) <= 0) {
...
int sample_battery_pack_current(Battery_sampling *battery_sampling) {
	...
	// TODO Remove cast (uint16_t*) ???
	if (readRegistersWithRetry(PACK_CURRENT_REGISTER_0, 2,
			(uint16_t*) &battery_sampling->pack_current, MODBUS_RETRY_COUNT) <= 0) {
		result = -1;
	} else {
		battery_sampling->pack_current_sum += battery_sampling->pack_current.fcurrent;
		battery_sampling->pack_current_count++;

		battery_sampling->min_pack_current = min(
				battery_sampling->min_pack_current, battery_sampling->pack_current.fcurrent);

		battery_sampling->max_pack_current = max(
						battery_sampling->max_pack_current, battery_sampling->pack_current.fcurrent);
	}
...
int load_battery_current(Battery_current *current) {
	...
	// TODO Remove cast (uint16_t*) ???
	if (readRegistersWithRetry(PACK_CURRENT_REGISTER_0, 2,
			(uint16_t*) &current->pack_current, MODBUS_RETRY_COUNT) <= 0) {
		result = -1;
	}

	// TODO Remove cast (uint16_t*) ???
	if (readRegistersWithRetry(DISCHARGE_OVERCURRENT_CUTOFF_REGISTER, 1,
			(uint16_t*) &current->discharge_overcurrent_cutoff, MODBUS_RETRY_COUNT) <= 0) {
		result = -1;
	}

	// TODO Remove cast (uint16_t*) ???
	if (readRegistersWithRetry(CHARGE_OVERCURRENT_CUTOFF_REGISTER, 1,
			(uint16_t*) &current->charge_overcurrent_cutoff, MODBUS_RETRY_COUNT) <= 0) {
		result = -1;
	}
...
int load_battery_temp(Battery_temp *battery_temp) {
	...
	if (readRegistersWithRetry(INTERNAL_TEMP_REGISTER, 1,
			(uint16_t*) &battery_temp->onboard_temp, 10) <= 0) {
		result = -1;
	}

	if (readRegistersWithRetry(EXTERNAL_TEMP_1_REGISTER, 1,
			(uint16_t*) &battery_temp->external_temp_1, 10) <= 0) {
		result = -1;
	}

	if (readRegistersWithRetry(EXTERNAL_TEMP_2_REGISTER, 1,
			(uint16_t*) &battery_temp->external_temp_2, 10) <= 0) {
		result = -1;
	}

	if (result == 1) {
		battery_temp->last_success = millis();
	}
...
int load_battery_config(Battery_config *config) {
	...
	if (readRegistersWithRetry(CELL_COUNT_REGISTER, 1,
			&config->cell_count, 10) <= 0) {
	...
	if (readRegistersWithRetry(PACK_CAPACITY_REGISTER, 1,
...
int load_battery_soc(Battery_soc *soc) {
	...
	// TODO Should this be uint32_t?
	if (readRegistersWithRetry(PACK_SOC_REGISTER_0, 2,
			(uint16_t*) &(soc->stateOfChargeHp), MODBUS_RETRY_COUNT) <= 0) {
		result = -1;
	} else {
...
int load_battery_soc2(Battery_soc *soc) {
	...
	if (readRegistersWithRetry(PACK_SOC2_REGISTER, 1,
			(uint16_t*) &soc->stateOfCharge, MODBUS_RETRY_COUNT) <= 0) {
		result = -1;
	}
...
int load_battery_safety(Battery_safety_params *safp) {
	...
	if (readRegistersWithRetry(CELL_CHARGED_VOLTAGE_REGISTER, 1,
			&safp->cell_charged_v, MODBUS_RETRY_COUNT) <= 0) {
		result = -1;
	}

	if (readRegistersWithRetry(CELL_DISCHARGED_VOLTAGE_REGISTER, 1,
			&safp->cell_discharged_v, MODBUS_RETRY_COUNT) <= 0) {
		result = -1;
	}

	DEBUGP("cell_charged_v=%u, cell_discharged_v=%u\r\n", safp->cell_charged_v, safp->cell_discharged_v);
...
int load_bms_version(Bms_version *ver) {
	...
	if (readRegistersWithRetry(BMS_VERSION_REGISTER_0, 10,
			(uint16_t*)ver, MODBUS_RETRY_COUNT) <= 0) {
		result = -1;
	}
